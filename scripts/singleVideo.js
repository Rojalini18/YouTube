...
function display(mainVideo, recommened) {
  ...
  //channel dp n subscriber details

  var channelicon = `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&key=${URL}&id=${mainVideo.snippet.channelId}`;
  fetch(channelicon)
    .then((res) => {
      return res.json();
    })
